...
- **目录**：支持按卷分组（`mode === "volumes"` 时渲染卷标题 + 卷下章节，其余按 `chapters` 平铺）；目录页支持**分页**（每页最多 100 章，顶部/底部翻页 + 跳页下拉框，兼容 `?page=N` 深链定位），几千章的长篇小说也不卡顿。
- **阅读页**：字号 A- / A+ 即时调节；正文经外部 `marked` 解析 Markdown 渲染。章节正文按**每页最多 1 万字**分页：短章显示「上一章 / 目录 / 下一章」，超长章（>1 万字）自动分页并显示「上一页 / 目录 / 下一页」，两组导航互斥、不同时出现。
